...
class ClinicalExamenSelector:
    ...
    def get_clinical_examens_by_symptom(symptom):
        ...
        return ClinicalExamen.objects.filter(
            ...
            Q(og__plaintes__og_symptom=symptom) |
            Q(od__plaintes__od_symptom=symptom) |
            Q(od__plaintes__og_symptom=symptom)
        ).distinct()
...
class StatsSelector:
    ...
    def get_common_symptoms(start_date, end_date, limit=5):
        ...
        od_symptoms = Plaintes.objects.filter(
            created__range=(start_date, end_date)
        ).values('od_symptom') \
         .annotate(count=Count('od_symptom')) \
         .order_by('-count')[:limit]

        og_symptoms = Plaintes.objects.filter(
            created__range=(start_date, end_date)
        ).values('og_symptom') \
         .annotate(count=Count('og_symptom')) \
         .order_by('-count')[:limit]

        return {
            'od_symptoms': od_symptoms,
            'og_symptoms': og_symptoms
        }